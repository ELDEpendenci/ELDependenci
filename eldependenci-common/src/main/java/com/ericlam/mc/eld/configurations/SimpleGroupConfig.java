...
public class SimpleGroupConfig<T extends GroupConfiguration> implements GroupConfig<T>, PreLoadable, FileLocator {
	...
	public synchronized List<String> listAll() {
		try {
			return fileWalker.walkAll(folder, (Predicate<Path>) null)
	...
	public synchronized List<T> findAll() {
		try {
			return fileWalker.walkAll(folder, (Predicate<Path>) null).map(Path::toFile).map(this::mapToInstance).toList();
		} catch (IOException e) {
			LOGGER.warn("Error while loading Folder {}{}", folder.toPath(), e.getMessage(), e);
		}
		return List.of();
	...
	private T mapToInstance(File f) {
		...
		try {
			...
			LOGGER.warn("Error while loading {}.yml: {}", id, e.getMessage(), e);
		}
		return null;
	}
	...
	public synchronized void save(T config) {
		...
		try {
			...
			LOGGER.warn("error while saving {}.yml: {}", config.getId(), e.getMessage(), e);
		}
	}
