...
public final class ELDCommandArgsHandler<Sender> {

    private final Map<Class<? extends Annotation>, ArgsHandler<Sender>> argsHandler = new LinkedHashMap<>();

    public <A extends Annotation> void registerArgAHandle(Class<A> annotation,
                                                          CommandArgHandler<A, Sender> handle,
                                                          Function<A, CommonProperties> propertyGetter
    ) {
        argsHandler.putIfAbsent(annotation,
                new ArgsHandler<Sender>(
    ...
    public Map.Entry<WideCommandArgHandler<Sender>, CommonProperties> getArgsEntry(Field field) {
        return argsHandler.entrySet()
                .stream()
                .filter(en -> field.isAnnotationPresent(en.getKey()))
                .findFirst()
                .map(en ->
                        new AbstractMap.SimpleEntry<WideCommandArgHandler<Sender>, CommonProperties>(
    ...
    private record ArgsHandler<Sender>(CommandArgHandler<Annotation, Sender> argsHandler,
